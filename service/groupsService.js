...
async function addExpense(groupId, userId, currency, comment, amount, shareOverview, exchangedAmount, exchangeRate) {
  ...
  const mongoInsert = {
    _userId: new ObjectId(userId), _groupId: groupId, currency, share_overview: shareOverview, comment, timestamp,
  };
  if (currency !== 'DKK') {
...
async function payDebt(group, payingMember) {
  ...
  const totalSurplus = membersOwed.reduce((total, member) => total + member.balance, 0);
